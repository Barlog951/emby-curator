...
def enrich_episodes_with_series_metadata(client: httpx.Client, base_url: str, episodes: List[Dict], library_id: Optional[str] = None) -> List[Dict]:
    """
    ...
    # Get series metadata for all series referenced by the episodes
    series_metadata = {}
    ...
    # Try to get User ID for user-specific API calls  
    user_id = None
    try:
        # Try to get user info if we have user credentials
    ...
    # Get series metadata for all unique series
    for series_id in unique_series_ids:
        try:
            ...
            series_metadata[series_id] = {
                ...
                "OriginalSeriesName": "Unknown Series"
            }
    ...
    for episode in episodes:
        ...
        if series_id and series_id in series_metadata:
            ...
            episode["SeriesName"] = "Unknown Series"
            episode["OriginalSeriesName"] = "Unknown Series"
...
def get_missing_episodes(client: httpx.Client, base_url: str, library_id: Optional[str] = None, username: Optional[str] = None, password: Optional[str] = None) -> List[Dict]:
    ...
    """
    # First, try the dedicated missing episodes endpoint with proper parameters
    url = f"{base_url}/Shows/Missing"
    
    # Build query parameters like the working curl request
    params = {
        "ImageTypeLimit": "1",
    ...
    # Try user authentication if available
    if username and password:
        ...
        try:
            ...
            auth_client, auth_token, user_id = create_http_client(base_url, username, password)
            logger.debug("Successfully authenticated for missing episodes")
            
            # Add UserId to params for authenticated requests
            if user_id:
                params["UserId"] = user_id
            
            # Try with authenticated client using proper parameters
            ...
            if response.status_code == 200:
                ...
                logger.debug(f"Missing episodes response type: {type(missing_data)}")
                ...
                # Enrich episodes with proper series metadata including original titles
                enriched_episodes = enrich_episodes_with_series_metadata(auth_client, base_url, episodes, library_id)
    ...
    try:
        # Set a longer timeout for missing episodes endpoint (it returns a lot of data)
        ...
        if response.status_code == 200:
            ...
            if isinstance(missing_data, dict) and "Items" in missing_data:
                ...
                logger.warning(f"Unexpected response format from missing episodes endpoint: {type(missing_data)}")
                logger.debug(f"Response content: {missing_data}")
                episodes = []
            
            logger.info(f"Found {len(episodes)} missing episodes via direct endpoint")
            
            # Enrich episodes with proper series metadata including original titles
            enriched_episodes = enrich_episodes_with_series_metadata(client, base_url, episodes, library_id)
            return enriched_episodes
    ...
    # Fallback: Try to find missing episodes using alternative approach
