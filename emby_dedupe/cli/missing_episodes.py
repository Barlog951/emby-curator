...
def format_missing_episodes_report(analysis_results: dict, format_type: str = "console") -> str:
    ...
    # Header
    report_lines.append("# Missing Episodes Report")
    report_lines.append("")
    
    # Statistics
    stats = analysis_results.get("statistics", {})
    report_lines.append("## Summary Statistics")
    report_lines.append(f"- **Total Missing Episodes**: {stats.get('total_missing_episodes', 0)}")
    report_lines.append(f"- **Series Affected**: {stats.get('total_series_affected', 0)}")
    report_lines.append(f"- **Seasons Affected**: {stats.get('total_seasons_affected', 0)}")
    
    if stats.get('most_missing_series'):
        report_lines.append(f"- **Series with Most Missing**: {stats.get('most_missing_series')}")
    
    if stats.get('average_missing_per_series'):
        avg = stats.get('average_missing_per_series', 0)
        report_lines.append(f"- **Average Missing per Series**: {avg:.1f} episodes")
    ...
    by_series = analysis_results.get("by_series", {})
    if by_series:
        report_lines.append("## Missing Episodes by Series")
        report_lines.append("")
        ...
        for series_name, series_data in sorted_series:
            report_lines.append(f"### {series_name}")
            report_lines.append(f"**Missing Episodes**: {series_data['total_missing']}")
            report_lines.append("")
            ...
            for season_num in sorted(episodes_by_season.keys()):
                ...
                if season_num > 0:
                    report_lines.append(f"**Season {season_num}**:")
                else:
                    report_lines.append("**Specials/Unknown Season**:")
...
def run_missing_episodes_command(args) -> None:
    ...
    """
    # Reuse existing environment variable handling
    ...
    override_warning("--verbosity", args.verbosity and logger.level, env_verbosity)
